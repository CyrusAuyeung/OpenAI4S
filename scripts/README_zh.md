...
| `check_css_tokens.py` | F-21：`openai4s/server/webui/style.css` 里 `var(--x)` 引用集减去 `--x:` 定义集必须为空。只用标准库、不联网。挂在 lint 作业的一个 step 上，同时也是本地 pre-commit hook。逐条报告缺失引用；未修的树还会打印 F-21 审计口径的 40 处硬 `--text-100`/`--text-300` 空洞。 |
| `extract_webui_contract.mjs` | F-01：机器扫描八个 `tests/browser_*.mjs` harness。把三份 E2E 兼容清单（裸 window 全局、`S` 字段读写面、DOM 选择器）写入 [`tests/webui-contract.md`](../tests/webui-contract.md)，并刷新 `tests/browser_smoke.mjs` 里的 `CONTRACT_GLOBALS` 逐名 `typeof` 前导。可重复运行、输出按名排序；`--check` 在任一文件漂移时失败，并点名被删掉的全局；`--self-test` 不需要 daemon。 |
...
| `render_skill_install_sections.py` | 渲染（或加 `--check` 校验）`skills/` 下每个 Skill 页面的 `## Install` / `## 安装` 一节：替换成该页自己的目录名、从 `cli.mjs` 读出的 Node 版本下限、从 `source.mjs` 读出的仓库名，以及集合的成员数。每种语言一份模板、逐页盖章，于是这九十个页面是一件可校验的产物，而不是九十处手改；`tests/test_skills_installer_contract.py` 会跑 `--check`，所以任何页面缺这一节、或被手改过的 Skill 目录都会让测试套件变红，直到重新运行本脚本。 |
...
| `verify_linux_bundle.py` | 只用标准库检查构建好的 `.tar.gz` 或已解包的目录。以下情况一律失败关闭：归档形状不对或启动器丢了可执行位、三处版本声明对不上、科学栈缺包、字节码是时间戳失效制、图标梯级的实际像素尺寸与目录名不符、`.desktop` 模板没人替换，或归档里混进了凭据material。在架构匹配的 Linux 主机上它还会多做一层——真正运行内嵌解释器，再把随包发布的 `install.sh` 装进一个临时 HOME（安装器正是用这个解释器渲染菜单项），读回渲染出的菜单项和 CLI 链接，最后运行 `uninstall.sh`——并在报告里写明这次达到了哪一层深度，而不是让「跳过的检查」看起来像「通过的检查」。 |
...
| `release_consistency.py` | 上传和最终发布前的只读交叉验证：实际分发件须与 provenance、SBOM、封存报告和构建收据一致；Windows 包须嵌入同架构 Linux 发布资产的精确字节，且 `payload/` 下不得有 launcher 可能改选的其他文件。无 staging 证明的手动最终发布也必须通过相同检查；若给出了 `--source-sha` / `--workflow-run-id`，封存的证据链必须与之一致。任何失败（包括损坏的归档）都以拒绝的形式呈现。SBOM、provenance 与证据包的文件名也只在这里定义一次。 |
