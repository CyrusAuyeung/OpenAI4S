...
The `openai` and `anthropic` wires stream token by token whenever the caller supplies a delta callback, and allow one blocking compatibility request only after HTTP 400/422 explicitly reports `streaming_not_supported`, or `unsupported_parameter` / `unknown_parameter` with structured `param: "stream"`. Authentication failures, empty streams, timeouts and uncertain read failures do not trigger compatibility requests. All transports share at most three sends and one backoff budget per logical call; the compatibility POST itself is attempted only once. Cancellation before a send prevents it, and received text, reasoning, tool fragments or usage veto transparent replay. `responses` is always SSE; `gemini` is always blocking. `OPENAI4S_LLM_STREAM=0` (also `false`/`no`/`off`) forces the blocking path on the two that choose — worth reaching for behind a proxy that mishandles SSE, at the cost of the reply arriving as one blob per turn.
