...
0.2.0 不检查它打开的数据库的 schema 版本。如果让它打开一个已经被 0.3.0 升级过的数据目录，它会不给任何警告地启动并写入这个数据库，但不会维护迁移 28 到 32 新增的任何内容。在一次测试中，0.2.0 运行在升级后的数据库副本上并没有造成损坏，因为这些迁移只做新增。但没有任何机制保证两个版本对它们都会写的那些行有一致的理解。
...
* **工作台是新的。** 默认 UI 是 Preact/TypeScript 工作台。`OPENAI4S_WEBUI=legacy` 仍可打开旧的 `app.js` UI，但它不再增加新功能；`OPENAI4S_WEBUI_NEXT` 会被忽略。
