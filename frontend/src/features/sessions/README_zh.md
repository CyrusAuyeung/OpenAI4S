...
| [`boot.ts`](boot.ts) | window 导出、`setLoadSessionsImpl`、工作台点击接线。Shell 的事件立即绑定，但首个视图要等语言分块加载完成（或加载失败、或超过 `I18N_ROUTE_WAIT_MS`）才路由：仪表盘列表、侧栏和打开的会话经 `t()` 渲染，字典到达后不会重绘。 |
| [`boot.i18n-gate.test.ts`](boot.i18n-gate.test.ts) | 字典加载前事件已绑定，首次路由等待字典；语言分块加载失败或卡住时仍会路由。 |
...
| [`dom.ts`](dom.ts) | `$` / `el` / `ago` / `navURL` / composer 辅助。`setTitle` 从静态 `data-i18n-val` 标签手中接管 `#conv-title`。 |
