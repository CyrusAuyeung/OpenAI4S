...
| [`projects.navigation.test.ts`](projects.navigation.test.ts) | A superseded project navigation cannot replace the current session or its session/folder lists; menu filtering cancels pending project opens without retiring current conversation reads, including repeated A→B→A filters; the current navigation transfers ownership to its conversation. |
