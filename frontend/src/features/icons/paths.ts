...
export function iconSvg(name: string, size?: number, cls?: string): string {
  const px = size || 16;
  const paths = Object.prototype.hasOwnProperty.call(ICON_PATHS, name) ? ICON_PATHS[name] : "";
  return (
    `<svg class="ic-svg${cls ? " " + cls : ""}" width="${px}" height="${px}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">` +
    paths +
    "</svg>"
  );
}
