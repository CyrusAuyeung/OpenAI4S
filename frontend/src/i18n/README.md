...
| [`runtime.ts`](runtime.ts) | `t` / `tOptional` / `setLang` / `applyStaticI18n` / `planModePayload`. The first dictionary load repaints static labels exactly as a language switch does. |
...
| [`static-i18n-race.test.ts`](static-i18n-race.test.ts) | Locale chunks held behind a gate: static labels applied before they load keep their readable fallback, and are repainted (with the language toggle and hooks) once they land. |
