...
| [`runtime.ts`](runtime.ts) | `t` / `tOptional` / `setLang` / `applyStaticI18n` / `planModePayload`。首次加载字典后，按切换语言的方式重绘静态标签。 |
| [`index.ts`](index.ts) | 给后续 F 系列工作项的公开导出。 |
| [`i18n.test.ts`](i18n.test.ts) | 键集对齐、抽取结果与 app.js 的 diff、`t()` 语义、计划模式 payload。 |
| [`static-i18n-race.test.ts`](static-i18n-race.test.ts) | 把语言分包挡在闸门后：字典到达前应用的静态标签保留可读的兜底文字，字典到达后连同语言切换按钮和语言钩子一起重绘。 |
