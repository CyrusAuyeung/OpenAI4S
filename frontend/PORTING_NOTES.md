...
| `app.js:216-227` `refreshThemeToggle` | `refreshThemeToggle` | `#dash-theme` / `#ws-theme` `data-icon` sun/moon, repainted from `features/icons/paths.ts` (paintIcons skips painted nodes, so swapping `data-icon` alone froze the boot glyph). Title via `tOptional`, so an unloaded dictionary never writes the bare key. `bindWorkbench` syncs it once the Shell exists and registers it as a language hook, as app.js did. |
...
| `applyStaticI18n` (161-167) | `runtime.ts` `applyStaticI18n` | `data-i18n` / `-title` / `-ph` / `-val`. A node with no translation yet keeps its markup text (app.js's dictionaries were synchronous; these are `import()` chunks the Shell usually mounts before). The first dictionary load re-runs the same repaint as `setLang`: static labels, `.lang-btn`, language hooks. |
...
| `renderArtifactBody` + 10 renderer glues 8637-8962 | `renderers.ts` | Sequence / MSA / genome / chemistry-2d / latex / markdown / table / text / download / sheet. image / pdf / html-preview / molecule-3d call F-18 islands through `isReady`. PDF stays `sandbox=""`; HTML starts inert and the scoped preview service upgrades it after a validated grant. |
...
| html-preview iframe 8664 | `frames.ts` + `artifacts/preview.ts` | Starts with `sandbox=""` and an app-origin preview. A validated Host-bound grant upgrades to the alternate loopback origin; failures keep the noscript note. Exact-version selections remain pinned. |
| `openKetcher` 10834 + workbench 8918 | `islands/ketcher.ts` | iframe `src` is app-origin `/ketcher` (optional `?artifact_id=`). `allow="clipboard-read; clipboard-write"`. **No sandbox** — `/ketcher` is first-party, served with embeddable headers (`frame-ancestors 'self'`). |
