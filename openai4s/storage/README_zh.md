...
| [`like.py`](like.py) | 各仓储共用的那一个子串 `LIKE` 模式：`\`、`%`、`_` 转成字面量，内嵌的 NUL 换成一个私用区替身——SQLite 把模式当 C 字符串读，遇到 NUL 就停；此前三份逐字节相同的私有副本让 DataPro 索引里的 NUL 修复从未抵达 project 与文件名搜索，于是 `lab\x00zzz` 会命中所有含 `lab` 的行。 |
