...
- `run` 在进程内跑完，动作路由与完成判定用的是与本地 Agent facade 相同的一套 Engine 规则。`run --auto` 是 Auto Mode：越界动作交给 Guardian 处理而不是直接失败关闭，结果也要先经过复核，run 才报告终态 —— CI 得能分清「跑完且验证过」和「跑完但没人检查」。复核读的是这次运行记录下来的内容（`--auto` 会记录它的 cell：代码、输出、报错，外加原生工具账本）；缺失的证据记为遗漏，绝不当作已验证。这只覆盖根运行自己的 cell：委派子代理的 cell 记在子代理自己的 frame 下，既不收集也不声明为遗漏，与 Web 路径一致。这不是完全放权；Guardian 真正生效的面是一份只读允许名单，且绑定在经过校验的动作摘要上。`run --mode reusable_pipeline|codebase_change` 只有在测试证据背后有一张 Host 授权的 `host.bash` 回执时才算完成；无头运行里没人能批准那条 shell 命令，所以 `--allow-test-command CMD`（可重复）只为本次运行预先授权这一条精确命令；既没有它、也没有常设规则或无人值守策略时，运行会在第一次调用模型之前直接拒绝。
- `run` 只有在任务真正完成（`stop_reason` 为 `submitted`）时才以 0 退出。以其他任何方式结束的 run —— `max_turns`、`no_progress`、`cancelled`，或 CLI 从没见过的停止原因 —— 一律以 3 退出，`--json` 照样打印完整结果，停止原因仍然读得到。2 仍表示拒绝执行（用法错误、标准环境未就绪、数据库比当前版本新），1 表示带 traceback 的未处理异常。加 `--auto` 时退出码同样只看 `stop_reason`，复核结论在 `auto_mode.terminal` 里。这张表也写在 `openai4s run --help` 中。以前任何终态都返回 0，于是检查 `$?` 的包装脚本会把打转或撞上轮数上限的 run 当成成功，而 Action Ledger 记下的却是失败。
