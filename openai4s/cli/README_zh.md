...
- 根 `run` 打开的 frame 由这次 run 自己收尾：完成记 `done`，取消或 Ctrl-C 记 `cancelled`，其余情况（包括抛异常）记 `failed`，并把 provider 的 token 总数写到这一行上。以前它永远停在 `processing`、token 为 NULL，同一数据目录上的 daemon 查到的也就是这个状态。委派子 Agent 的 frame 仍归委派 runner 管。
