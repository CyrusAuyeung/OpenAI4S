...
- `run` 在进程内跑完，动作路由与完成判定用的是与本地 Agent facade 相同的一套 Engine 规则。`run --auto` 是 Auto Mode：越界动作交给 Guardian 处理而不是直接失败关闭，结果也要先经过复核，run 才报告终态 —— CI 得能分清「跑完且验证过」和「跑完但没人检查」。复核读的是这次运行记录下来的内容（`--auto` 会记录它的 cell：代码、输出、报错，外加原生工具账本）；缺失的证据记为遗漏，绝不当作已验证。这只覆盖根运行自己的 cell：委派子代理的 cell 记在子代理自己的 frame 下，既不收集也不声明为遗漏，与 Web 路径一致。这不是完全放权；Guardian 真正生效的面是一份只读允许名单，且绑定在经过校验的动作摘要上。`run --mode reusable_pipeline|codebase_change` 只有在测试证据背后有一张 Host 授权的 `host.bash` 回执时才算完成；无头运行里没人能批准那条 shell 命令，所以 `--allow-test-command CMD`（可重复）只为本次运行预先授权这一条精确命令字符串；既没有它、也没有常设规则或无人值守策略时，运行会在第一次调用模型之前直接拒绝。那张回执是内核 worker 自己报告的、这条字符串在 Cell 环境（`PATH`、cwd、进程内状态）里运行后的退出码：它能抓住没跑过、只是打印出来或者跑失败了的测试，但并不能证明 Cell 没有蓄意伪造测试运行器。
- `run` 只有在任务真正完成（`stop_reason` 为 `submitted`）时才以 0 退出。以其他任何方式结束的 run —— `max_turns`、`no_progress`、`cancelled`，或 CLI 从没见过的停止原因 —— 一律以 3 退出，`--json` 照样打印完整结果，停止原因仍然读得到。2 仍表示拒绝执行（用法错误，比如任务为空 `empty_task`、`--allow-test-command` 无效 `invalid_allow_test_command`；显式代码模式下没有任何途径能授权它的测试命令 `code_mode_test_runner_unauthorized`；标准环境未就绪；数据库比当前版本新 —— `--json` 会打印这个 code），1 表示带 traceback 的未处理异常。加 `--auto` 时退出码同样只看 `stop_reason`，复核结论在 `auto_mode.terminal` 里。这张表也写在 `openai4s run --help` 中。以前任何终态都返回 0，于是检查 `$?` 的包装脚本会把打转或撞上轮数上限的 run 当成成功，而 Action Ledger 记下的却是失败。
- 根 `run` 打开的 frame 由这次 run 自己收尾：完成记 `done`，取消或 Ctrl-C 记 `cancelled`，其余情况（包括抛异常）记 `failed`，并把 provider 的 token 总数写到这一行上。frame 已经建好、却在第一次调用模型之前就被拒绝的 run（显式代码模式下没有任何途径能授权它的测试命令）同样记 `failed`。以前它永远停在 `processing`、token 为 NULL，同一数据目录上的 daemon 查到的也就是这个状态。委派子 Agent 的 frame 仍归委派 runner 管。
...
- 正因为这道门开着，`serve`、`url` 打印给人看的 URL 会带上 `?token=…`，否则 SPA 在能给出任何入口之前就先收到 401。凡是不交到人手上的用法，取的都是同一个 URL 的不带 token 版本：凭据不该出现在日志行或窗口标题里。`status` 就属于这种用法 —— 它是健康检查，输出会进 CI 和支持日志 —— 所以它只打印不带 token 的地址，并提示用 `openai4s url` 获取登录 URL；以前它会把完整 token 打印出来。
...
- `serve` 先绑定端口，再打印横幅，这样端口冲突表现为一次失败的启动，而不是「启动成功之后的崩溃」。`stop` 只宣称它验证过的事：它轮询等待进程真正退出，`--timeout`（默认 30 秒）用完时 daemon 仍在收尾，就以退出码 2 报错、pidfile 原样留下 —— 过早清掉状态文件，正是当初一个还活着、还占着端口的 daemon 被 `status` 判成「没在跑」、又被下一个 `serve` 一头撞上的原因；`stop --force` 在同样的轮询之后升级为 SIGKILL。超过 5 秒的 SIGTERM 宽限期后，它会向 stderr 打印一行 `shutting down…` 并继续轮询：正在收尾首个 kernel 的 bootstrap 或多个活 kernel 的 daemon 需要 8-9 秒，在 5 秒处放弃，会把一次自己就能完成的关闭误报成失败。
