...
| [`project_listing.py`](project_listing.py) | 从 gateway 里搬出来的 `GET /projects`：参数表（`q`、`limit`、`cursor`、`offset`）及其稳定的 400 码、不透明 keyset 分页（取 `limit + 1` 行来观察 `has_more`）、`?offset` 兼容窗口，以及无参数时的完整列表——全部走 `Store` 门面。团队可见性仍是仓储 SQL 里的 WHERE 合取项；本模块从不对一页做事后过滤。返回的是原始行，由 gateway 自己做投影，因此这里不 import `gateway`。 |
