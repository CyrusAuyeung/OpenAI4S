...
| [`font_cache.py`](font_cache.py) | 强制沙箱里的内核启动时拿到的 matplotlib 字体列表。这种沙箱给每个内核一个空的私有 `MPLCONFIGDIR`，于是每个新内核第一次画图都要从头重建字体列表——在 macOS 上要 8-48 秒，因为 matplotlib 会去调 `system_profiler`。如果做成内核可写的共享缓存，或者把跑完的内核重建出的列表再拷出来复用，就等于让一个 Cell 去投毒另一个会话的内核要解析的文件；所以这份列表只由 host 构建：用内核自己的解释器，经 `preinstall.run_confined_probe` 受限运行，执行一段固定程序、写进一个全新的空缓存目录。这个探针在自己的空工作目录里运行，而且那段程序在做任何其他导入之前，先把 `python -c` 放在 `sys.path` 最前面的工作目录项去掉；所以 daemon 启动目录（它可能就是某个 CLI 内核的工作区）里的 `json.py` 或 `matplotlib/` 决定不了它打印什么。host 校验它打印出来的内容，存到 `<data_dir>/cache/matplotlib-fonts/<解释器 key>/` 下，manager 在 worker 启动之前，通过拒绝 symlink 的目录描述符把它拷进每个新沙箱的私有临时目录。位于内核自己工作区里的缓存一律不信任；只有字体目录的指纹仍然吻合时才复用，所以新装的字体会被下一个内核看到；而且只有构建它的那份 `font_manager.py` 仍保持构建程序报告的大小和修改时间时才复用——matplotlib 按自己的版本给列表命名，所以升级 matplotlib 之后会触发重建，而不是在一份看上去仍然有效的缓存后面，让每个内核各自扫描一个月。只有正在服务的 daemon（`run_server`）会在后台启动构建；CLI 单次运行和测试只读取已有缓存。daemon 关停时也会停掉仍在跑的构建（`shutdown_background_builds`）：还没拉起构建程序的构建不会再拉起；正在跑的构建程序整个进程组先收到 SIGTERM（构建程序借此删掉自己的临时目录），短暂宽限后再发 SIGKILL，它拉起的每个进程都会被杀掉，包括 macOS `system_profiler` 在自己单独的进程组里重新拉起的那个辅助进程；然后最多等几秒让构建线程结束，好删掉探针工作区。绝不会等扫描跑完；以前 `openai4s stop` 报告成功之后，扫描还挂在 PID 1 下面继续跑。任何一次未命中都只会退回到原来内核里自己扫描字体的做法，绝不会让内核起不来。 |
| [`guards.py`](guards.py) | 探测从一个 Cell 漏到下一个 Cell 的状态：Cell 打开却没关掉的 pyplot figure，以及少数进程级全局注册表——它们会在 Cell 之前被 pin 住，之后再做 diff。这些只是廉价的探测，不是隔离手段。对应的可选库不在时，这一项不做任何事；`OPENAI4S_GUARDS_OFF=1` 则把整套关掉。figure 探测只在 Cell 自己已经导入过 pyplot 时才去读它：由这里去导入，会让每个新内核的第一个 Cell（连 `x = 1` 也算）都白付一次 matplotlib 的字体扫描。 |
...
| [`preinstall.py`](preinstall.py) | 内核这边的包管理，刻意隔着一层：它是辅助工具，标准库核心不会对它形成硬导入依赖。启动阶段只**报告**科学基线里缺了什么；把这份计划真正执行掉是另一回事，得由人或某个显式 API 主动要求。以前 `serve` 会自己执行它：在后台线程上把约 23 个未固定版本的包名拿去 PyPI 解析，再带 `--break-system-packages` 装下去——于是启动 daemon 就改动了用户的解释器，相隔一周的两次冷启动结果不一致，而离线的那次会在没人看着的地方失败。新装的包只有新进程才看得到，所以装完之后重启内核是调用方的事。完全没有 `pip` 模块的解释器——uv 构建的 virtualenv 默认就不带——会退回到 PATH 上的 `uv` 可执行文件、并指向同一个解释器去装；退回的只是**工具选择**，pip 在场时装失败就按原样报告，不会再换 uv 重试一遍。每个外来解释器都要走的受限探测也归它（超时时结束探针以及它拉起的一切——它的进程组，以及像 `system_profiler` 的扫描辅助进程那样离开了该进程组的后代；只杀组长会让这些活继续跑；它还接受一个 `stop` 事件，别的线程设置之后就能提前结束探针，而不必等这类辅助进程仍然占着的管道），还有 `freeze_for`：它只从指定的那一个解释器读出包列表，读不到就什么都不返回，绝不退回到当前进程自己的那份——Artifact 的环境溯源正是照着它来归属的。 |
| [`protocol.py`](protocol.py) | JSON Lines 单帧的共享字节上限。worker 生产端与远端 socket 接收端导入同一个值，因此一端明确放行的 response 不会在另一端又被当成超限拒绝。 |
| [`provenance.py`](provenance.py) | 在 Python worker 内部安装对象级的血缘插桩：经受支持的读取路径读进来的对象，会带上来源 Artifact 的 `version_id`；这些对象后来被写出去时，它们累积的输入版本会被上报。它能看到多少算多少，不声称看得全。给那些「挂不住属性」的对象用的旁表，除了 id 之外还把对象本身一起存着：id 不等于身份，CPython 会立刻复用刚释放掉的地址，于是一个毫不相干的对象就继承了原先住在那个地址上的对象的血缘。这张表有大小上限，按最先进入的先淘汰——淘汰只会丢掉一条标记，而不会凭空造出一条。它从不为了插桩而主动导入 matplotlib：一个 `sys.meta_path` 钩子在 Cell 导入 `matplotlib.figure` 的那一刻包装 `Figure.savefig`，所以在同一个 Cell 里导入、画图、保存依然会上报血缘边，而从不画图的 Cell 也就不必付字体扫描的代价。 |
| [`r_kernel.py`](r_kernel.py) | 解析出真实的 `Rscript`，并构造文件描述符安全的命令，让 R sibling 通过公共 manager 跑起来；绝不会被静默换成 Python。 |
| [`r_worker.R`](r_worker.R) | 常驻的 R worker，execute/response 契约与 Python 完全一致：输出捕获、中断、出错的行号与调用、资源计量。一个 Cell 的两条流会以 `blocking = TRUE` 写进 host 那侧的 fifo——R 的 `fifo()` 默认是非阻塞的，管道缓冲区塞不下的部分会被悄悄丢掉——因此节奏由 host 的读取方来控制，输出是边跑边到，而不是全堆到最后；response 里再置上 `sink_capture`，说明这个 Cell 的输出握在 host 手里，不在 worker 手里。顶层表达式之间仍然会 flush 一次，为的是那种结束时还没把缓冲区填满的表达式。入站 frame 用 `jsonlite` 解析，出站 JSON 却是手写转义的——所以即便这套 R 里没装 `jsonlite`，它也能报出一个结构化的干净错误，而不是直接死掉。它是分析通道：没有 `host` 对象，没有 Cell 中途 RPC，也没法从 Cell 内部完成任务。worker 自己的东西没有一个是 `.GlobalEnv` 里的绑定：整个文件跑在一个私有环境里（父环境是 `.GlobalEnv`，所以用户自己写的 S3 `print`/`conditionMessage` 方法照样能被分派到），`quit()`/`q()` 的拦截挂在附加到搜索路径上的 `openai4s:guards` 环境里——因此 Cell 里执行 `rm(list = ls(all.names = TRUE))` 只会清掉用户的变量，别的一概不动，变量检查器也不做任何过滤，原样列出 `.GlobalEnv` 的每个绑定。 |
