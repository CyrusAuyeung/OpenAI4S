...
| [`transport.py`](./transport.py) | Stdlib JSON/SSE transport over urllib. A CallState created by client.chat carries at most three sends, shared backoff spending and cancellation across the stream and its single compatibility POST. Every send checks cancellation first. Only structured connection refusal or a retryable refusal without semantic output permits replay; uncertain read failures never do. SSE errors retain response headers, request IDs and Retry-After. Invalid non-empty JSON events fail explicitly. Legacy four-argument JSON and five-argument SSE injection still work. |
